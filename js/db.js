  ...
  return new Promise((resolve, reject) => {
    ...
    req.onupgradeneeded = (event) => {
      ...
      if (!db.objectStoreNames.contains("weeklyProgressEvents")) {
        const s = db.createObjectStore("weeklyProgressEvents", {
          keyPath: "id",
        });
        s.createIndex("trackId", "trackId", { unique: false });
        s.createIndex("trackId_week", ["trackId", "weekNumber"], {
          unique: false,
        });
      }
      ...
      if (!db.objectStoreNames.contains("dailyProgressEvents")) {
        const s = db.createObjectStore("dailyProgressEvents", {
          keyPath: "id",
        });
        s.createIndex("trackId", "trackId", { unique: false });
      ...
      if (!db.objectStoreNames.contains("longTermGoalAssessments")) {
        const s = db.createObjectStore("longTermGoalAssessments", {
          keyPath: "id",
        });
