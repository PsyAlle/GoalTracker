 ...
 */
export function openWeeklyGoalForm({
  periodId,
  currentWeek,
  track,
  existingVersion,
  ...
  openModal((close) => {
    const nameInput = el("input", {
      type: "text",
      value: existingVersion?.name || "",
      placeholder: "T.ex. Moonboard",
    });
    const targetInput = el("input", {
      type: "number",
      min: "1",
      value: String(existingVersion?.targetValue ?? 3),
    });
    const unitInput = el("input", {
      type: "text",
      value: existingVersion?.unit || "pass",
      placeholder: "pass, set, ...",
    });
    const descInput = el("textarea", {
      value: existingVersion?.description || "",
      placeholder: "Beskrivning / anteckningar (valfritt)",
    });
 ...
 */
export function openDailyGoalForm({
  periodId,
  fromDate,
  track,
  existingVersion,
  ...
  const links = existingVersion ? [...existingVersion.links] : [];
  const weekdays = existingVersion
    ? [...existingVersion.weekdays]
    : [1, 2, 3, 4, 5];
  ...
  openModal((close) => {
    const nameInput = el("input", {
      type: "text",
      value: existingVersion?.name || "",
      placeholder: "T.ex. Split stretch",
    });
    const targetInput = el("input", {
      type: "number",
      min: "1",
      value: String(existingVersion?.targetValue ?? 5),
    });
    const descInput = el("textarea", {
      value: existingVersion?.description || "",
      placeholder: "Beskrivning / anteckningar (valfritt)",
    ...
    const form = el("div", {}, [
      ...
      el("div.field", {}, [el("label", { text: "Namn" }), nameInput]),
      el("div.field", {}, [
        el("label", { text: "Målvärde per dag" }),
        targetInput,
      ]),
...
export function openLongTermGoalForm({ goal, onSaved }) {
  ...
  openModal((close) => {
    const nameInput = el("input", {
      type: "text",
      value: goal?.name || "",
      placeholder: "T.ex. Göra en muscle-up",
    });
    const descInput = el("textarea", {
    ...
    const form = el("div", {}, [
      el("h2", {
        text: goal ? "Redigera långsiktigt mål" : "Nytt långsiktigt mål",
      }),
      el("div.field", {}, [el("label", { text: "Namn" }), nameInput]),
      el("div.field", {}, [el("label", { text: "Beskrivning" }), descInput]),
      el("div.field", {}, [
