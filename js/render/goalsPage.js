...
import { getAllLongTermGoals, archiveLongTermGoal, unarchiveLongTermGoal, latestAssessment } from '../domain/longTermGoals.js';
import { openWeeklyGoalForm, openDailyGoalForm, openLongTermGoalForm } from './goalForms.js';
...
export async function renderGoalsPage(container) {
  ...
  if (period && period.status === 'active') {
    ...
    root.appendChild(el('p', { text: 'Ingen period skapad ännu.', class: 'empty-state' }));
  }
...
async function renderLongTermSection() {
      ...
      el('div.goal-card', {}, [
        el('div.goal-card-top', {}, [
          el('div', {}, [
            ...
            goal.description ? el('div.goal-meta', { text: goal.description }) : null,
            latest ? el('div.goal-meta', { text: `Senaste skattning: ${latest.score}/10` }) : null,
          ]),
        ]),
        el('div.goal-card-actions', {}, [
          el('button', {
            text: 'Redigera',
            onClick: () => openLongTermGoalForm({ goal, onSaved: () => refreshPage() }),
          }),
          el('button', {
            text: 'Arkivera',
            onClick: async () => {
  ...
  if (active.length === 0) {
    list.appendChild(el('p', { text: 'Inga aktiva långsiktiga mål.', class: 'goal-meta' }));
  }
  ...
  if (archived.length > 0) {
    ...
    for (const goal of archived) {
      archList.appendChild(
        el('div.goal-card', {}, [
          el('div.goal-card-top', {}, [el('div.goal-name', { text: goal.name })]),
          el('div.goal-card-actions', {}, [
            el('button', {
              text: 'Återaktivera',
              onClick: async () => {
...
async function renderWeeklySection(period, currentWeek) {
  ...
  const section = el('div', {}, [
    ...
    el('button.btn.block', {
      text: '+ Nytt veckomål',
      onClick: () =>
        openWeeklyGoalForm({ periodId: period.id, currentWeek, onSaved: () => refreshPage() }),
    }),
  ]);
  ...
  for (const track of tracks) {
    ...
    list.appendChild(
      el('div.goal-card', {}, [
        el('div.goal-card-top', {}, [
          el('div', {}, [
            el('div.goal-name', { text: `${latest.name} — ${latest.targetValue} ${latest.unit}` }),
            latest.description ? el('div.goal-meta', { text: latest.description }) : null,
          ]),
        ]),
        el('div.goal-card-actions', {}, [
          el('button', {
  ...
  if (tracks.length === 0) list.appendChild(el('p', { text: 'Inga veckomål ännu.', class: 'goal-meta' }));
  section.appendChild(list);
  return section;
}
...
async function renderDailySection(period, todayDate) {
  ...
  const section = el('div', {}, [
    ...
    el('button.btn.block', {
      ...
      onClick: () =>
        openDailyGoalForm({ periodId: period.id, fromDate: todayDate, onSaved: () => refreshPage() }),
    }),
  ]);
  ...
  if (tracks.length === 0) list.appendChild(el('p', { text: 'Inga dagliga mål ännu.', class: 'goal-meta' }));
  section.appendChild(list);
  return section;
}
