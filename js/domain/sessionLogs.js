 ...
 */
export async function addSessionLog(periodId, dateStr, { goalRef = null, readiness, rpe, note = '' }) {
  const log = {
    id: newId(),
    periodId,
    date: dateStr,
...
export async function getSessionLogsForPeriod(periodId) {
  ...
  return logs.sort((a, b) => (a.date === b.date ? a.createdAt - b.createdAt : a.date < b.date ? 1 : -1));
}
...
export async function resolveGoalRefName(goalRef) {
  ...
  if (goalRef.kind === 'weekly') {
    const versions = await store.byIndex('weeklyGoalVersions', 'trackId', goalRef.id);
    if (!versions.length) return null;
    return versions.sort((a, b) => a.effectiveFromWeek - b.effectiveFromWeek).pop().name;
  }
  if (goalRef.kind === 'daily') {
    const versions = await store.byIndex('dailyGoalVersions', 'trackId', goalRef.id);
    if (!versions.length) return null;
    return versions.sort((a, b) => (a.effectiveFromDate < b.effectiveFromDate ? -1 : 1)).pop().name;
  }
...
export async function getGoalRefOptions(periodId) {
  ...
  const weeklyTracks = await store.byIndex('weeklyGoalTracks', 'periodId', periodId);
  for (const track of weeklyTracks) {
    const versions = await store.byIndex('weeklyGoalVersions', 'trackId', track.id);
    if (!versions.length) continue;
    const latest = versions.sort((a, b) => a.effectiveFromWeek - b.effectiveFromWeek).pop();
    options.push({ kind: 'weekly', id: track.id, label: `${latest.name} (veckomål)` });
  }

  const dailyTracks = await store.byIndex('dailyGoalTracks', 'periodId', periodId);
  for (const track of dailyTracks) {
    const versions = await store.byIndex('dailyGoalVersions', 'trackId', track.id);
    if (!versions.length) continue;
    const latest = versions.sort((a, b) => (a.effectiveFromDate < b.effectiveFromDate ? -1 : 1)).pop();
    options.push({ kind: 'daily', id: track.id, label: `${latest.name} (dagligt mål)` });
  }

  const longTermGoals = await store.byIndex('longTermGoals', 'status', 'active');
  for (const goal of longTermGoals) {
    options.push({ kind: 'longterm', id: goal.id, label: `${goal.name} (långsiktigt mål)` });
  }
