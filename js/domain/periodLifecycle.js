...
export async function runLifecycleCheck() {
  ...
  while (changed) {
    ...
    const autoPlanning = periods.find(
      (p) => p.status === 'planning' && p.planningEndDate && today > p.planningEndDate && wasAutoCreated(p)
    );
    if (autoPlanning) {
      autoPlanning.status = 'active';
      autoPlanning.startDate = addDays(autoPlanning.planningEndDate, 1);
      autoPlanning.endDate = addDays(autoPlanning.startDate, ACTIVE_DURATION_DAYS - 1);
...
async function copyGoalTemplates(fromPeriodId, toPeriodId) {
  const oldWeeklyTracks = await store.byIndex('weeklyGoalTracks', 'periodId', fromPeriodId);
  for (const track of oldWeeklyTracks) {
    const versions = await store.byIndex('weeklyGoalVersions', 'trackId', track.id);
    if (versions.length === 0) continue;
    const latest = versions.sort((a, b) => b.effectiveFromWeek - a.effectiveFromWeek)[0];

    const newTrackId = newId();
    await store.put('weeklyGoalTracks', { id: newTrackId, periodId: toPeriodId, createdAt: Date.now() });
  ...
  const oldDailyTracks = await store.byIndex('dailyGoalTracks', 'periodId', fromPeriodId);
  ...
  for (const track of oldDailyTracks) {
    ...
    if (versions.length === 0) continue;
    const latest = versions.sort((a, b) => (a.effectiveFromDate < b.effectiveFromDate ? 1 : -1))[0];

    const newTrackId = newId();
    await store.put('dailyGoalTracks', { id: newTrackId, periodId: toPeriodId, createdAt: Date.now() });
...
export async function getCurrentPeriod() {
  const periods = await store.all('periods');
  return periods.find((p) => p.status === 'active' || p.status === 'planning') || null;
}

export async function getClosedPeriods() {
  const periods = await store.all('periods');
  return periods.filter((p) => p.status === 'closed').sort((a, b) => b.createdAt - a.createdAt);
}
...
export async function periodForDate(dateStr) {
  ...
  return (
    periods.find(
      (p) => (p.status === 'active' || p.status === 'closed') && p.startDate && p.startDate <= dateStr && dateStr <= p.endDate
    ) || null
  );
}
